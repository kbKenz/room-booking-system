...
function BookingForm({
  ...
  const handleEndDate = (dateArray) => {
    console.log("handleEndDate - Input dateArray:", dateArray);
    let recurringEndDate = [];
    dateArray.forEach((item) => {
      recurringEndDate.push(parseInt(item));
    });
    console.log("handleEndDate - Output recurringEndDate:", recurringEndDate);
    return recurringEndDate;
  };

  // Format the recurring data into an array
  const handleRecurringData = (type, date) => {
    console.log("handleRecurringData - Input type:", type, "date:", date);
    let recurringData = [];
    if (type !== "none") {
      recurringData = [date, type];
      console.log(
        "handleRecurringData - Creating recurring data:",
        recurringData
      );
    } else {
      recurringData = [];
      console.log("handleRecurringData - No recurring booking");
    }
    console.log("handleRecurringData - Final output:", recurringData);
    ...
    if (startTime && endTimeSelect) {
      ...
      if (!found) {
        ...
        for (let i = 0; i < endTimeSelectOptions.length; i++) {
          ...
          const [optHours, optMinutes] = optionValue
            .split(":")
            .map((num) => parseInt(num, 10));
    ...
    <Fragment>
      <div className="header__page">
        <h2 className="header__heading header__heading--sub">
          Level {roomData.floor} | {roomData.name}
        </h2>
      </div>
      <form
        ...
        onSubmit={(event) => {
          ...
          // Booking specifics
          let recurringEnd = [];
          let recurringType = formData.recurring.value;
          console.log("Form recurring type:", recurringType);

          // Only process recurring end date if recurring is selected
          if (recurringType !== "none") {
            if (!formData.recurringEndDate.value) {
              alert(
                "Please select a recurring end date when using recurring bookings."
              );
              return;
            }

            recurringEnd = handleEndDate(
              formData.recurringEndDate.value.split("-")
            );
            console.log(
              "Form recurringEndDate value:",
              formData.recurringEndDate.value
            );
            console.log(
              "Form recurringEndDate split:",
              formData.recurringEndDate.value.split("-")
            );
            console.log("Processed recurringEnd:", recurringEnd);

            // Adjust recurring end date month to be consistent with booking dates (0-11 format)
            if (recurringEnd.length >= 2) {
              recurringEnd[1] = recurringEnd[1] - 1;
              console.log("Adjusted recurringEnd month:", recurringEnd);
            }
          }

          let recurringData = handleRecurringData(recurringType, recurringEnd);
          console.log("Final recurringData for submission:", recurringData);

          const purpose = formData.purpose.value;
          const description = formData.description.value;

          console.log("Submitting booking with description:", description);
          console.log("ALL BOOKING DATA being submitted:", {
            startDate,
            endDate,
            purpose,
            roomId,
