...
class App extends Component {
  ...
  onDeleteBooking = (roomId, bookingId) => {
    console.log(
      `Attempting to delete booking: roomId=${roomId}, bookingId=${bookingId}`
    );
    ...
    console.log(
      `Calling deleteBooking with roomId=${roomIdStr}, bookingId=${bookingIdStr}`
    );
  ...
  loadMyBookings = () => {
    ...
    const userId =
      this.state.decodedToken.sub ||
      this.state.decodedToken.user_id ||
      this.state.decodedToken.id;
    const userEmail =
      this.state.decodedToken.email || this.state.decodedToken.user_email;
    ...
    if (!userId && !userEmail) {
      console.log(
        "WARNING: No user ID or email found in token!",
      ...
      if (room.bookings && Array.isArray(room.bookings)) {
        ...
        if (room.bookings.length > 0) {
          console.log(
            ...
            JSON.stringify(room.bookings[0])
          );
        }
        ...
        room.bookings.forEach((booking) => {
          ...
          const bookingUserId =
            booking.user || booking.userId || booking.UserId || booking.user_id;
